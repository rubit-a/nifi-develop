...
# EC2 배포 스크립트
# 사용법: ./deploy-to-ec2.sh <EC2_HOST> [EC2_USER]
#
# 예시:
#   ./deploy-to-ec2.sh ec2-xx-xx-xx-xx.compute.amazonaws.com
#   ./deploy-to-ec2.sh ec2-xx-xx-xx-xx.compute.amazonaws.com ec2-user
...
if [ $# -lt 1 ]; then
    ...
    echo "  $0 ec2-xx-xx-xx-xx.compute.amazonaws.com ec2-user"
    echo ""
    exit 1
fi
...
if [ ! -f ~/.ssh/id_rsa ]; then
    echo "⚠️  Warning: SSH key not found at ~/.ssh/id_rsa"
    echo "   Make sure you have SSH access to the EC2 instance"
    echo ""
fi
...
# NAR 파일을 EC2로 전송
scp "${NAR_FILE}" "${EC2_USER}@${EC2_HOST}:/tmp/"
